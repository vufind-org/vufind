...
public class FormatCalculator
{
    ...
    protected boolean isMap(Record record) {
        // No clue what Doug is doing.
		DataField field336 = (DataField) record.getVariableField("336");
        if (field336 != null) {
            if (field336.getSubfield('a') != null) {
                if (field336.getSubfield('a').getData().toLowerCase().contains("cartographic image")) {
                    return true;
                }
            }		
		}
    ...
    protected boolean isManuscript(Record record) {
        // No clue what Doug is doing.
		DataField field336 = (DataField) record.getVariableField("336");
        if (field336 != null) {
            if (field336.getSubfield('a') != null) {
    ...
    protected List<String> getFormatsAsList(Record record) {
        ...
        if (formatFromRecordType.length() > 0) {
			if (formatFromRecordType == "Manuscript") {
				couldBeBook = false;
			}
