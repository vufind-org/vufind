...
class MapTest extends \PHPUnit_Framework_TestCase
{
     ...
     */
    function testGetBib(){
        $bib = Bib::find(8594968, $this->mockAccessToken);
    ...
    {
        $this->assertContains('schema:Map', $bib->types());
    }
    
    /**
     *@vcr bibIllustrator
     */
    function testGetBibIllustrator(){
        $bib = Bib::find(8594968, $this->mockAccessToken);
        $this->assertInstanceOf('WorldCat\Discovery\CreativeWork', $bib);
        foreach ($bib->getIllustrators() as $illustrator){
            $this->assertThat($illustrator, $this->logicalOr(
                $this->isInstanceOf('WorldCat\Discovery\Person'),
                $this->isInstanceOf('WorldCat\Discovery\Organization')
            ));
        }
