...
class DVDTest extends \PHPUnit_Framework_TestCase
{
    ...
    function testParseResources($bib){
        ...
        foreach ($bib->getActors() as $actor){
            $this->assertInstanceOf('WorldCat\Discovery\Person', $actor);
        }
        
        $this->assertInstanceOf('WorldCat\Discovery\Person', $bib->getDirector());
        
        foreach ($bib->getMusicBy() as $person){
            $this->assertInstanceOf('WorldCat\Discovery\Person', $person);
        }
        
        foreach ($bib->getProducers() as $person){
            $this->assertInstanceOf('WorldCat\Discovery\Person', $person);
        }
