...
We recommend you make a copy of the `local_theme_example` directory, inside of the `themes` directory. That way, when there are updates to VuFind, you won't run into trouble with the updates changing your customizations or causing conflicts in git.
...
Setting up themes under `alternate_themes` allows you to add a `ui=name` parameter to the end of your URL to see that theme. For example, the default configuration of `custom:local_theme_example` would cause `https://vufind.org/?ui=custom` to show the theme described in the directory `local_theme_example`.
...
This is configured in `theme.config.php`. LESS inheritance is targetted towards and controlled by the [`compiled.less`](https://github.com/vufind-org/vufind/blob/master/themes/local_theme_example/less/compiled.less) file.
