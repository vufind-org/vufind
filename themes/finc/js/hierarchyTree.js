 ...
 * opens full hierarchy tree by default #14067
 * remove file icon, insert via SCSS
 configured in: {}
 */
...
function buildJSONNodes(xml) {
  ...
  $(xml).children('item').each(function xmlTreeChildren() {
    ...
    jsonNode.push({
      ...
      a_attr: {
        href: name.attr('href'),
        // finc: removes redundant title attribute - refs #18621
        // title: name.text()
      },
...
$(document).ready(function hierarchyTreeReady() {
  ...
  $("#hierarchyTree")
    .bind("ready.jstree", function jsTreeReady(/*event, data*/) {
      ...
      // finc: opens full hierarchy tree by default #14067 - CK
      // tree._open_to(htmlID);
      tree.open_all();
    ...
    .jstree({
      ...
      types: {
        record: {
          // finc: removes file icon, insert via SCSS -- CK
