/**
 ...
 usage: {}
 modified for finc:
 * add / remove fields and groups #18277
 * set focus on new group or field - FIXME: Remove after PR is effective - #17985 - RL
 * exported addGroup, addSearch, deleteGroup, deleteSearch
 * add else for barf #17985, #19811
 * check for aria-label #19651
 * accessibility changes #23451
 * adapt adv search #19811
 * Resetting Bootstrap Sliders #23451
 configured in: {}
 */
...
function addSearch(group, _fieldValues, isUser = false) {
  ...
  // finc: adds / remove fields and groups #18277
  ...
  // finc: adds else for barf #17985, #19811
  } else {
...
function addGroup(_firstTerm, _firstField, _join, isUser = false) {
  ...
  // finc: checks for aria-label #19651
  if ($newGroup.attr('aria-label')) {
    // finc: adds / remove fields and groups #18277
...
$(document).ready(function advSearchReady() {
  $('.clear-btn').click(function clearBtnClick() {
    $('input[type="text"]').val('');
    // finc: accessibility changes #23451
    $('input[type="number"]').val('');
    // finc: adapts adv search #19811
    $('select').not('#limit').find('option:selected').prop("selected", false);
    if (typeof defaultLimit !== 'undefined') {
      $("#limit").val(defaultLimit);
    }
    // finc: adapts adv search #19811 - END
    // finc: Resetting Bootstrap Sliders #23451
    ...
    // finc: Resetting Bootstrap Sliders #23451 - END
