 ...
 * use specific translation keys for tooltips in account menu #23517
* remove class 'holds-status' #23517
configured in: {}
*/
...
$(document).ready(function registerAccountAjax() {
  ...
  VuFind.account.register("checkedOut", {
    ...
    render: function render($element, status, ICON_LEVELS) {
      ...
      if (status.warn > 0) {
        // finc: use specific translation keys for tooltips in account menu #23517
  ...
  VuFind.account.register("mediareadytopickup", {
    ...
    render: function render($element, status, ICON_LEVELS) {
      ...
      if (status.available > 0) {
        ...
        // finc: removes class 'holds-status' #23517
        $element.addClass("hidden");
      }
      $('[data-toggle="tooltip"]', $element).tooltip();
      return level;
  ...
  // finc: add handler for permanent loans #23285 - END
  ...
  // finc: show total count of ill request items #23285 - END
});
