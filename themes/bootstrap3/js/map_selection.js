...
function loadMapSelection(geoField, boundingBox, baseURL, homeURL, searchParams, showSelection, resultsCoords) {
  ...
  init = function drawMap() {
     ...
     map.on('click', function displayPopup(evt) {
         ...
         function showFeature(feature, clusterLayer) {
         ...
         if (popupfeature) {
             ...
             if ((fLayerId === 'clusterLayer') && (fType === 'object') && (cFeatures.length < 5)) {
                 ...
                 for (var i = 0; i < cFeatures.length; i++) {
                     var cFeatureName = cFeatures[i].get('name');
                     var cFeatureId = cFeatures[i].get('id');
                     var cFeatureContent = '<article>' + cFeatureName.link(homeURL+'Record/'+cFeatureId) + '</article>';
     ...
     map.on('pointermove', function changeMouseCursor(evt) {
         ...
         if (hit) {
            var fl = map.forEachFeatureAtPixel(pixel, function getFeature(feature, clusterLayer) {
                return {'feature':feature, 'layer':clusterLayer};
            });
