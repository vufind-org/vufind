...
function checkSaveStatuses() {
  var data = $.map($('.result'), function(i) {
    return {'id':$(i).find('.hiddenId')[0].value, 'source':$(i).find('.hiddenSource')[0].value};
  });
  if (data.length) {
    var ids = [];
    var srcs = [];
...
$(document).ready(function() {
  ...
  // attach click event to the save record link  
	$('.save-record').click(function() {
		$('#myModalLabel').html('Save to Favorites');
		$('#modal').modal({'remote':'/AJAX/JSON?method=getLightbox&submodule=Record&subaction=Save&id='+$(this).attr('id')});
	}).attr('href','#');
});