...
/**
 * Abtract base class for maintenance actions.
 *
 * PHP version 8
 *
 ...
 * @link     https://vufind.org Main Site
 */
abstract class AbstractMaintenanceAction extends AbstractAdminAction
{
    /**
     * Constructor.
     *
    ...
    protected function getScripts(): array
    {
        ...
        $config = $this->configManager->getConfigArray('AdminScripts');
        $globalConfig = $config['Global'] ?? [];
        unset($config['Global']);

        // Filter out any commands that the current user does not have permission to run:
        $permission = $this->getHelper(PermissionHelper::class);
