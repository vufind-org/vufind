...
class HomeAction extends AbstractAdminAction
{
    ...
    public function action(
        ...
        $xml = false;
        if ($url = $this->config['Index']['url'] ?? null) {
            $solrResponse = $this->httpService->get($url . '/admin/cores?wt=xml');
            $xml = $solrResponse->isSuccess() ? $solrResponse->getBody() : false;
        }

        return $this->renderTemplate(
            $request,
            $response,
            ['xml' => $xml ? simplexml_load_string($xml) : false]
        );
    }
}
