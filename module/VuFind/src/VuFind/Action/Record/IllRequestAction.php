...
class IllRequestAction extends AbstractRecordAction implements TranslatorAwareInterface
{
    ...
    public function action(
        ...
        $gatheredDetails = $illRequestsHelper->validateRequest($request, $checkRequests['HMACKeys']);
        if (!$gatheredDetails) {
            return $this->redirectToRecord();
        }

        // Block invalid requests:
        $validRequest = $this->ilsConnection->checkILLRequestIsValid(
            $driver->getUniqueID(),
            $gatheredDetails,
            $patron
        );
        ...
        if ((is_array($validRequest) && !$validRequest['valid']) || !$validRequest) {
            $flashMessagesHelper->addErrorMessage(
                is_array($validRequest) ? $validRequest['status'] : 'ill_request_error_blocked'
            );
        ...
        if (null !== $this->getPostParam('placeILLRequest')) {
            ...
            if ($results['success'] ?? false) {
                $msg = [
                    ...
                    'tokens' => [
                        '%%url%%' => $this->routeHelper->getUrlFromRoute('myresearch-illrequests'),
                    ],
                ];
        ...
        $defaultRequiredDate = $illRequestsHelper->getDefaultRequiredDate($checkRequests);
        ...
        return $this->renderTemplate($request, $response, $templateParams, 'record/illrequest');
    }
}
