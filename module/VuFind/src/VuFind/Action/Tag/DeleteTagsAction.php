 ...
 * @link     https://vufind.org Main Site
 */
class DeleteTagsAction extends AbstractTemplateRenderingAction
{
    /**
     * Constructor.
     *
    ...
    public function action(
        ...
        if ($this->getHelper(FormHelper::class)->formWasSubmitted($request, ['deleteSelectedtag'])) {
            if (!$this->csrf->isValid($this->getPostParam('csrf'))) {
                throw new \VuFind\Exception\BadRequest('error_inconsistent_parameters');
            }
        }

        if ($tags = $this->getPostParam('deleteSelectedtag', [])) {
            $this->resourceTagsService->deleteLinksByResourceTagsIdArray($tags);
            $this->tagService->deleteOrphanedTags();
        }

        return $this->getHelper(RedirectHelper::class)->redirectToRoute($response, 'tag-userlist');
    }
}
