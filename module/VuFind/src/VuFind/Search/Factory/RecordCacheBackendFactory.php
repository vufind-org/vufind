...
class RecordCacheBackendFactory implements FactoryInterface
{
    ...
    protected function createRecordCollectionFactory()
    {
        ...
        $recordFactories['RDSIndex'] = function ($data) use ($manager) {
            $driver = $manager->get('RDSIndex');
            $driver->setRawData($data);
            $driver->setSourceIdentifier('RDSIndex');
            return $driver;
        };
        
        $recordFactories['RDSProxy'] = function ($data) use ($manager) {
            $driver = $manager->get('RDSProxy');
            $driver->setRawData($data);
            $driver->setSourceIdentifier('RDSProxy');
            return $driver;
        };
