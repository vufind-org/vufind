...
class SpecBuilder
{
     ...
     * Constructor
     *
     * @param array $spec      Settings for specific lines.
     * @param array $itemSpecs Settings for lines generated by a record driver's getItems method.
     */
    public function __construct(protected array $spec = [], protected array $itemSpecs = [])
    {
    ...
    /**
     * Set additional options for items that match a specific value in
     * a specific key.
     *
     * @param string $itemKey   Key of the item
     * @param string $itemValue Value of the item
     * @param array  $options   Additional options
     *
     * @return void
     */
    public function setItemLine(string $itemKey, string $itemValue, array $options): void
    {
        $this->itemSpecs['options'][$itemKey][$itemValue] = $options;
    }
    ...
    public function addItems(array $defaultOptions = [], array $filter = []): void
    {
        if (!isset($defaultOptions['startPos'])) {
            $this->maxPos += 1000;
            $defaultOptions['startPos'] = $this->maxPos;
        }
        if (!isset($defaultOptions['renderType'])) {
            $defaultOptions['renderType'] = 'RecordDriverTemplate';
        }
        if ($defaultOptions['renderType'] === 'RecordDriverTemplate' && !isset($defaultOptions['template'])) {
            $defaultOptions['template'] = 'data-item.phtml';
        }
        $this->itemSpecs = ['enabled' => true, 'defaultOptions' => $defaultOptions, 'filter' => $filter];
    }
